...
public class Board {
	...
	private int Reds, Blues, Greens, Assassins;
	private int totalLocations;
	
	 //Instantiates the Board class with the roles, teams, and grid

	public Board(Location[][] grid, int Reds, int Blues, int Greens, int Assassins, String RedName, String BlueName, String GreenName) {
		this.grid=grid;
		this.Red = new Team(RedName);
		this.Red.setAgents(Roles.Red);
		this.Blue = new Team(BlueName);
		this.Blue.setAgents(Roles.Blue);
		this.Green = new Team(GreenName);
		this.Green.setAgents(Roles.Green);
		...
		for(int i = 0; i < grid.length; i++)
		{
			for(int j = 0; j < grid.length; j++) {	
				...
				if(grid[i][j].isRevealed() && grid[i][j].getPerson().getRole().equals(Roles.Red)) 
					redsum++;
				if(grid[i][j].isRevealed() && grid[i][j].getPerson().getRole().equals(Roles.Green)) 
					greensum++;
			}
		}
	...
	/**
	 * This method returns amount of revealed assassins.
	 * @return Amount of revealed assassins
	 */
	public int isAssassinRevealed() {
		int ass = 0;
		for(int i = 0; i < grid.length; i++)
		{
			for(int j = 0; j < grid.length; j++) {	
	...
	/**
	 * Returns the number of Green Team members.
	 * @return number of Green.
	 */
	...
	public int getBystanders() {
		return (getGrid().length*getGrid().length)-(Reds+Blues+Greens+Assassins);
	}
